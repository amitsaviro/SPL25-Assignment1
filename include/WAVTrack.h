 ...
 *
 * Phase 4 contracts:
 * - load(): simulate deck preparation for WAV (often faster due to no
 * decompression).
 * - analyze_beatgrid(): run immediately after load() in this assignment; can be
 * more precise.
 * - get_quality_score(): derived from sample_rate and bit_depth (higher =>
 * better).
 * - clone(): return a deep polymorphic copy used by the mixer; source remains
 * unchanged.
 * - get_quality_score(): function of sample_rate and bit_depth (both higher ->
 * better).
 */
class WAVTrack : public AudioTrack {
 private:
  int sample_rate;  // Samples per second: 44100 (CD), 48000 (pro), 96000+
                    // (hi-res)
