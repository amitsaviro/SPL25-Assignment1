...
// - After loading to a deck: call track.load(); then analyze_beatgrid(); then
// switch active deck.
...
class MixingEngineService {
 ...
 public:
   ...
   * - @return: index of the deck the track was loaded to (0 or 1), or -1 on
   * failure.
   * - @brief: This function clones the track, unloads the target deck if
   * needed, loads the new track, analyzes the beatgrid, switches the active
   * deck, and unloads the previous deck.
   ...
   * - @brief This function calculates average BPM between active deck and given
   * track, then sets the given track's BPM to the average
   * - @attention What should be the preconditions of this function? What this
   * method modifies?
   */
   ...
   *
   */
  void set_auto_sync(bool enabled) { auto_sync = enabled; }

  /**
   * @brief Set BPM tolerance for mixing compatibility
   *
   */
  void set_bpm_tolerance(int tolerance) { bpm_tolerance = tolerance; }
};
