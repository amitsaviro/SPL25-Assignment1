...
class DJControllerService {
 public:
  ...
  // Output: An integer indicating the result: 1 for HIT, 0 for MISS without
  // eviction, -1 for MISS with eviction.
   ...
   * @note This function is meant for a single usage. don't call it more then
   * once.
   */
   ...
   * @return A raw pointer to the track if found, otherwise nullptr. Does not
   * transfer ownership.
   */
