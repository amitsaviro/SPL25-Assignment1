...
int DJSession::load_track_to_controller(const std::string& track_name) {
  ...
  if (result == 1) {  // YA cache HIT
    ...
    std::cerr << "[ERROR] Unexpected cache return code: " << result
...
bool DJSession::load_track_to_mixer_deck(const std::string& track_title) {
  std::cout
      << "[System] Delegating track transfer to MixingEngineService for: "  // YA information msg
...
void DJSession::simulate_dj_performance() {
  ...
  if (play_all) {
    for (const std::string& pl_name : playlist_names) {
      // Y - NOT NEEDED: std::cout << "\n[System] Auto-processing playlist: " <<
      // pl_name << "\n";
      ...
      if (!load_playlist(pl_name)) {
        ...
        ++stats.errors;
        continue;  // YA if we dont succeed, error and continue to the next
                   // playlist
      }

      // Y - not sure if needed
      //    std::cout << "[System] Playlist '" << pl_name << "' loaded with "
      ...
      for (const std::string& title : track_titles) {
        ...
        if (cache_result != 1 && cache_result != 0 && cache_result != -1) {
          std::cerr << "[ERROR] Unexpected cache result (" << cache_result
                    << ") for track: " << title << "\n";
          ++stats.errors;
          continue;  // YA every result that is not allowed-error and continue
                     // to the next
        }
  ...
  // YA iter status-default, play_all=false
    ...
    while (true) {
      ...
      if (selected_playlist.empty()) {  // YA empty choose=break
        std::cout
            << "[System] Session cancelled by user or no playlist selected.\n";
        break;
      }

      if (!load_playlist(
              selected_playlist)) {  // YA try to load the choosen playlist
      ...
      //              << "' loaded with " << track_titles.size()
      //              << " tracks.\n";

      for (const std::string& title : track_titles) {  // YA for every track
        std::cout << "\n-- Processing: " << title << " --\n";

        ++stats.tracks_processed;  // YA counting every track we tried to
                                   // proccess

        int cache_result =
            load_track_to_controller(title);  // YA the same proccess like (A)
        if (cache_result != 1 && cache_result != 0 && cache_result != -1) {
          std::cerr << "[ERROR] Unexpected cache result (" << cache_result
                    << ") for track: " << title << "\n";
          ++stats.errors;
