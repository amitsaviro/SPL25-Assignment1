...
bool del = true;  // hint: what is the purpose of this variable? how it changes
                  // the ownership semantics?
// YA track is the owner of itself-created and deleted in the main and not in
// the PlayList
void test_phase_1_memory_leaks() {
  std::cout << "\n======== PHASE 1: MEMORY LEAK TESTING ========" << std::endl;
  std::cout << "Testing the broken Playlist class..." << std::endl;

  // Create some tracks (these will leak if AudioTrack Rule of 5 isn't
  // implemented)
  std::cout << "Creating tracks..." << std::endl;
  AudioTrack* mp3 =
      new MP3Track("Sunset Lover", {"Petit Biscuit"}, 180, 120, 320);
  AudioTrack* wav = new WAVTrack("Strobe", {"Deadmau5"}, 645, 128, 44100, 16);

  // Create playlist and add tracks (this will leak nodes if destructor is
  // broken)
...
void test_phase_3() {
  ...
  try {
    ...
    PointerWrapper<MP3Track> smart_track(
        new MP3Track("Smart Track", {"AI Artist"}, 240, 130, 320));

    // Test access operators (will be TODO messages initially)
    std::cout << "\nTesting access operators..." << std::endl;
    if (smart_track) {
      std::cout << "Smart pointer contains: " << smart_track->get_title()
                << std::endl;
    ...
    std::cout << "Original pointer after move: "
              << (smart_track ? "still valid" : "null") << std::endl;
    std::cout << "Moved pointer: " << (moved_track ? "valid" : "null")
              << std::endl;

    // Test reset
    std::cout << "\nTesting reset..." << std::endl;
    moved_track.reset(
        new MP3Track("Reset Track", {"Reset Artist"}, 180, 125, 256));

    // Test release
    std::cout << "\nTesting release..." << std::endl;
    MP3Track* raw_ptr = moved_track.release();
    std::cout << "Released pointer: "
              << (raw_ptr ? raw_ptr->get_title() : "null") << std::endl;
...
void demonstrate_polymorphism() {
  ...
  tracks.push_back(std::unique_ptr<AudioTrack>(
      new MP3Track("MP3 Example", {"Artist 1"}, 200, 128, 320)));
  tracks.push_back(std::unique_ptr<AudioTrack>(
      new WAVTrack("WAV Example", {"Artist 2"}, 180, 130, 44100, 16)));
  ...
  for (auto& track : tracks) {
    ...
    std::cout << "Quality: " << track->get_quality_score()
              << std::endl;  // Virtual function call
...
int main(int argc, char* argv[]) {
  ...
  if (run_software) {
    ...
    std::cout << "=================================================="
              << std::endl;
    std::cout << "This program tests all phases of the assignment."
              << std::endl;
    ...
    std::cout << "\n(Set 'run_software' to true in main.cpp to run the full "
                 "interactive session.)\n"
              << std::endl;
  }
