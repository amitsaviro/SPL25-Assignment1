...
MixingEngineService::MixingEngineService()
    : decks{nullptr, nullptr},  // YA both deacks are empty
      ...
      auto_sync(false),  // the auto bpm sync is getting false for default
...
MixingEngineService::~MixingEngineService() {
  std::cout
      << "[MixingEngineService] Cleaning up decks...\n";  // YA cleaning msg

  for (AudioTrack*& deck : decks) {  // YA checking both
    if (deck != nullptr) {
      delete deck;  // YA delete the Track of this deck-deck is the ownership of
                    // the AudioTrack
...
int MixingEngineService::loadTrackToDeck(const AudioTrack& track) {
  ...
  if (!clone) {
    std::cerr << "[ERROR] Track: \""
              << track.get_title()  // error msg if clone is fail
              << "\" failed to clone\n";
    return -1;
  }
  ...
  size_t target = first_load ? 1 : (1 - active_deck);  // target deck
                                                       // else: inactive deck
  ...
  if (decks[target] != nullptr) {
    ...
    std::cout << "[Unload] Clearing previous track from deck " << target << ": "
              << old_title << "\n";
  ...
  if (!first_load && auto_sync && can_mix_tracks(clone)) {
    sync_bpm(clone);
  }
  ...
  if (!first_load && active_deck != target &&
      decks[active_deck] !=
          nullptr) {  // YA if the target one isnt the active one
  ...
  displayDeckStatus();  // print the current status
 ...
 */
bool MixingEngineService::can_mix_tracks(
    const PointerWrapper<AudioTrack>& track) const {
  if (!track ||
      decks[active_deck] == nullptr) {  // YA if there is no active deck or
                                        // track we cant mix-false
    return false;
  }
  ...
  int diff = abs(active_bpm - incoming_bpm);  // YA difference - Absolute
 ...
 */
void MixingEngineService::sync_bpm(
    const PointerWrapper<AudioTrack>& track) const {
  if (!track ||
      decks[active_deck] ==
          nullptr) {  // YA if there is no active deck or track we cant sync
    return;
  }
  ...
  std::cout << "[Sync BPM] Syncing BPM from " << incoming_bpm << " to "
            << avg_bpm << "\n";  // information msg
}
