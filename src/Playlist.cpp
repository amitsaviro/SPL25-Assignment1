...
void Playlist::copy_from(
    const Playlist& other) {  // YA helper function for the copy constructor
  PlaylistNode* current_src = other.head;
  PlaylistNode** tail = &head;  // YA connection point between the lists, set
                                // the pointer on the head

  while (current_src) {
    *tail = new PlaylistNode(
        current_src
            ->track);  // YA take the next Node from other with his pointer and
                       // create a new one in our list on the next place
...
Playlist& Playlist::operator=(
    const Playlist& other) {  // YA copy assignment operator
  if (this == &other) {
...
void Playlist::display() const {
  ...
  while (current) {
    ...
    std::for_each(artists.begin(), artists.end(),
    ...
    std::cout << index << ". " << track->get_title() << " by " << artist_list
              << " (" << track->get_duration() << "s, " << track->get_bpm()
              << " BPM)" << std::endl;
    current = current->next;
    index++;
  }
