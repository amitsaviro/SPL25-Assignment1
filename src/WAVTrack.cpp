...
WAVTrack::WAVTrack(const std::string& title,
                   const std::vector<std::string>& artists, int duration,
                   int bpm, int sample_rate, int bit_depth)
    : AudioTrack(title, artists, duration, bpm),
      sample_rate(sample_rate),
      bit_depth(bit_depth) {
  std::cout << "WAVTrack created: " << sample_rate << "Hz/" << bit_depth
            << "bit" << std::endl;
}
...
void WAVTrack::analyze_beatgrid() {
  ...
  // should print "  → Estimated beats: <beats>  → Precision factor: 1.0
  // (uncompressed audio)"
  double beats_estimated = (duration_seconds / 60.0) * bpm;
  const float precision_factor = 1.0;
...
PointerWrapper<AudioTrack> WAVTrack::clone() const {
  ...
  return PointerWrapper<AudioTrack>(
      nullptr);  // Replace with your implementation
}